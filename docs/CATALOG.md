...
| `docs/evidence/reach_1/findings.md` | REACH-1: Graph reachability analysis. For each GT person, can a connected path through D1 edges produce correct identity? Production: 0/8 independently reachable (267/357 hops are concurrent-node flicker), 8/8 joint feasibility. Dedup-ceiling: 2/8 reachable, 5/8 joint (6 shared SOLO nodes = detection ceiling). All 15/13 not-selected edges are capacity-blocked, 0 cost-beaten. Tool: `tools/reach_1_analysis.py`. | **CURRENT** |

...
| `docs/evidence/piece12_results/findings.md` | Piece 12: VFR redaction rendering. cv2.VideoWriter replaced by PyAV (rate=90000, source 90kHz timebase). CFR defect FIXED: DB/media gap 2.272s→0.466s (0.466s = 7 missing frames, pre-existing). mpeg4→h264 (−35% file size). PTS tick histogram exact match. Guard test at rate=15 demonstrated failing. `fps` parameter removed. `av>=18.0` declared. **Piece 12 COMPLETE.** | **CURRENT** |
| `docs/evidence/piece7_results/findings.md` | Piece 7: Stage F output format (Shape 3 hybrid). Plain path: VFR via `-fps_mode passthrough -enc_time_base -1`. Redacted path: CFR divergence FIXED by Piece 12. Sites #12 deleted (dead), #14/#15 fixed (sidecar `nominal_fps`). #13 deferred to Piece 9 (debug viz). GOP snap unchanged. **Piece 7 COMPLETE.** | **CURRENT** |
...
| `docs/evidence/gt_diag_1/findings.md` | GT-DIAG-1: GT-to-pipeline sequence diagnostic. Three-layer (tracklet/D1-node/person) contiguous-segment view with purity, group-span IoU, edge-cost analysis. 63% of node boundaries have no edge (graph-construction gap). Module: `src/pipeline_validation/gt_sequence_diag/`. | **CURRENT** |
| `docs/evidence/reach_1/findings.md` | REACH-1: Graph reachability analysis. For each GT person, can a connected path through D1 edges produce correct identity? Production: 0/8 independently reachable (267/357 hops are concurrent-node flicker), 8/8 joint feasibility. Dedup-ceiling: 2/8 reachable, 8/8 joint. 0 structural impossibilities in both arms (Hungarian matching prevents frame-level co-occupancy). All 15/13 not-selected edges are capacity-blocked, 0 cost-beaten. The solver has never made a cost mistake on this clip. Tool: `tools/reach_1_analysis.py`. | **CURRENT** |

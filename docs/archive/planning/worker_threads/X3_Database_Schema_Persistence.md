# X3 — Database Schema + Persistence
...
## Module-specific context (X3 — database)
Persist outputs for later retrieval and compliance.

### Must include
- Schema:
  - athletes (athlete_id, name, opt_in)
  - tags (tag_id -> athlete_id)
  - matches (match_id, athlete_a, athlete_b, start/end, clip_id, confidence)
  - clips (path, crop params, redaction status)
  - audit_events
- Migration/init strategy (simple for POC)

...
| Session match_sessions overwrite bug | Fixed | Same class of bug as export manifest overwrite. Stage E wrote all cameras to shared `match_sessions.jsonl` — last camera overwrote all others. Fix: `SessionStageLayoutAdapter.match_sessions_jsonl()` returns `match_sessions_{cam_id}.jsonl` (per-camera scoped). Processor merges per-camera match_sessions into shared `match_sessions.jsonl` after Loop 1. `session_f_run.py` prefers per-camera file, falls back to shared, and filters by `camera_id` as defense-in-depth. FP7oJQ recovered from 0 → 29 matches, J_EDEw from 0 → 106 matches (previously overwritten by PPDmUg's 4 matches). |
...
- **CP17 Tier 1: Implemented (2026-03-26).** Two-pass cross-camera ILP with tag corroboration. `cross_camera_evidence.py` builds evidence from Pass 1 identity assignments. `session_d_run.py` accepts `constraints_overlay` parameter to inject evidence between D2 and D3. `d3_ilp2.py` boosts MCF-2a miss penalty and MCF-3a must-link penalty by `corroboration_miss_multiplier` (default 10x) for corroborated tags. Processor inserts Pass 2 between cross-camera merge and Loop 2. Also fixed pre-existing bug: `_extract_must_link_groups_from_constraints()` treated D2's list-format `must_link_groups` as dict — must_link support was silently disabled. Validated on 3-camera data: J_EDEw tag-1 person_ids reduced 8→5 (37.5% reduction). Cross-camera merge still produces 0 links because all tag-1 person_ids have confidence=1.0 (single tag = 100% vote) — intra-camera dedup correctly skips ties. Next step: merge-level tie-breaking for single-tag scenarios, or test with distinct multi-tag data.
- **Next:** CP17 Tier 2/3 (coordinate evidence) stubbed for CP18. Full pipeline re-run to validate end-to-end CP17 + tie-breaking.
...
- **Last updated:** 2026-03-27 (Session match_sessions overwrite bug fixed — same class as export manifest overwrite; per-camera scoping + merge)

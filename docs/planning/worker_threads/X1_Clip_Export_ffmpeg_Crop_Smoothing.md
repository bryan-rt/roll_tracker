...
# X1 — Clip Export (ffmpeg) + Crop Smoothing
...
## Module-specific context (X1 — clip export)
Export match clips reliably.

### Must include
- Crop window: union of two athletes (boxes/masks) + padding
- Crop smoothing over time to avoid jitter
- ffmpeg command strategy (re-encode vs stream copy constraints)
- Deterministic filenames + manifest entries

...
| CP16b: Calibration pipeline skeleton + functional lens calibration | Completed | `src/calibration_pipeline/` created with `lens_calibration.py` (functional), `mat_walk.py`, `drift_detection.py`, `inter_camera_sync.py` (stubs → CP18). Lens calibration: interactive matplotlib tool loads existing 4-corner correspondences from homography.json, user clicks additional points along mat edges, edge-snapping auto-computes world coords via linear interpolation, `cv2.calibrateCamera` with `CALIB_FIX_TANGENT_DIST` estimates K + radial distortion (k1,k2). Writes `camera_matrix`, `dist_coefficients`, `lens_calibration` metadata to homography.json preserving all existing fields. `homography_calibrate.py` updated: (1) `_write_homography_json()` now read-then-merges to preserve K+dist across homography re-calibrations, (2) auto-undistorts frame at load when K+dist present in homography.json (both clicks and overlay_rect modes). Added `calibration_pipeline` to `pyproject.toml` packages. Two-step calibration chain: lens_calibration → K+dist, then homography_calibrate on undistorted frame → H valid for undistorted pixels. |
...
- **Next:** CP17 — Second ILP layer (tag signal, coordinate-ready).

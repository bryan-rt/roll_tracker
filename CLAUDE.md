...
| RECORDER-COVERAGE-1 + BACKLOG-1: Content delivery lag | **Fixed** | CP-R8 (2026-08-19, FP7oJQ): 11 segments, 1,134s footage in 2,552s elapsed. The "inter-segment gaps" were delivery lag, not lost footage — content is contiguous across all segment boundaries within an attempt (verified visually). The Nest relay delivers at 0.10–0.53× real-time (ffmpeg `speed=`, still climbing at termination — 0.53× is not a measured ceiling). The single discontinuity is at the attempt-1→3 boundary (reconnect loses the relay backlog). **Root cause:** ffmpeg `-t "$(( DEADLINE - now ))"` computes content duration from wall-clock remaining. Under arrival-PTS (pre-passthrough) this was accidentally correct (content time = wall time). Passthrough decoupled them, turning a coincidence into a defect. Expression unchanged since `745b1b4` (March). The March recorder was not better — tidy segment spacing was an artifact of the timestamp domain being the same as the clock domain, while CFR padded duplicates. **Fix (RECORDER-BACKLOG-1):** `TARGET_CONTENT_SECONDS` mode sets `-t` to min(content remaining, wall-clock remaining). Content accounting persists across attempts. Wall-clock safety cap (default 5× target, covers two reconnects with ramp cost). Legacy `WINDOW_SECONDS` preserved. **Reconnect cost:** each reconnect resets delivery speed to ~0.10× and must re-climb, costing wall time beyond the lost content. Evidence: `docs/evidence/recorder_coverage_1/findings.md`. |
| RECORDER-MUXER-PTS-1: Segment-start duplicate PTS | **Open — recorder fix needed** | The ffmpeg segment muxer emits a duplicate PTS on the first segment of each attempt: frame index 2 gets the same PTS as frame index 1. Deterministic (2 for 2 attempts in CP-R8, always frame 2, always attempt-first segment). Frames are NOT pixel-identical (distinct content, wrong PTS — this is a PTS stamping bug, not a frame duplicate). At 1/90000 timebase a 33ms interval is 2970 ticks and cannot round to zero, so CP-R13a is not at fault. Affects 200827 and 202356 (2 of 11 CP-R8 segments, 18%). Variable-dt tracker raises on `dt_s<=0` (correct — ratio 0.0 silently degrades tracking). **Decision: fix the muxer (enforce strictly monotonic PTS at segment boundaries) and re-capture the two affected segments. Do not annotate 200827 or 202356 until fixed.** The remaining 9 segments are clean. |

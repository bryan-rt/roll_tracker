...
| RECORDER-COVERAGE-1: Inter-segment gaps in passthrough recorder | **Open — investigation needed** | CP-R8 capture (2026-08-19, FP7oJQ single camera): 11 segments, 1,134s footage in 2,552s elapsed = **44.4% coverage**. Gaps of 42–338s between consecutive segments, including within healthy attempts (attempt 3: 8 segments, 864s footage, 1,623s span, 53.3% coverage — 759s lost to 7 inter-segment gaps, mean 108s). **Untested hypothesis:** the pre-passthrough recorder did not produce gaps of this magnitude (reported, not measured — comparison requires SOURCE_PTS=0 A/B on the same camera). **Consequence:** every capture before this is understood loses >50% of the session. Annotation (objective 6) is sequenced after the recorder investigation (objective 1). Evidence: `docs/evidence/recorder_coverage_1/findings.md`. |

...
`stages.stage_A.tracker.variable_dt` in `configs/default.yaml` (default **true** since
VDT-DEFAULT-1; correctness decision — a Kalman filter assuming uniform spacing on variable
footage is being fed a falsehood, regardless of aggregate score on one clip). When enabled:
...
- Track lifetime uses wall-time `max_lost_seconds` (default 2.0s). This is now a **live
  production parameter**. 2.0s matches stock 30-frame lifetime at ~67ms cadence (0.5%
  difference on FP7oJQ-132650); equivalence is **cadence-dependent** — a camera at different
  fps would diverge.
...
- `dt_s=0.0` valid (same-PTS frames on bimodal segments) → ratio 0.0 → position no-op.
  **Now on the production path** (VDT-DEFAULT-1). Guard at `tracker.py:180` accepts 0.0,
  rejects negative. Pre-MUXER-PTS-1 footage has duplicate PTS at frame 2; every such clip
...
| Coast architecture: variable-dt Kalman | **Default true (VDT-DEFAULT-1) — T3 result superseded by correctness decision** | Subclass (not fork) of boxmot's `KalmanFilterXYWH` and `BotSort` at `src/bjj_pipeline/tracking/`. Uses dt ratio (`dt_s / nominal_dt_s`) to keep velocity in px/nominal-frame. Both KF sites replaced (`self.kalman_filter` + `STrack.shared_kalman`). Track lifetime via wall-time `max_lost_seconds` (default 2.0s, now live production parameter, cadence-dependent equivalence) — `frame_rate` eliminated. Default `variable_dt: true` in `configs/default.yaml` since VDT-DEFAULT-1 (2026-09-01). T1 PASS. T2 PASS. Piece 8 dissolved. **T3 result (2026-08-30):** correct_id 37.2% → 34.3%, -2.9pp on FP7oJQ-20260822-132650. **SUPERSEDED (VDT-DEFAULT-1):** measurement stands, conclusion does not — uniform-dt assumption is a falsehood on variable-spacing footage regardless of aggregate score on one clip at 2.1% dispersion. See `docs/evidence/piece11_t3/findings.md` §10. `dt_s=0.0` guard now on production path. Both T3 arms at `outputs/_t3_arms/`. Process-noise dt-scaling is a recorded follow-up. |

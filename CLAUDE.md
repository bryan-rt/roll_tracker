...
| Cross-camera ILP enhancement | Planned | Current merge is post-hoc (links resolved person_ids after per-camera ILP). Future: feed cross-camera tag co-observations back as must_link constraints into per-camera D3 ILP before solving — enables resolving within-camera identity swaps using cross-camera evidence. Options: enrich original ILP, second ILP pass, or global ILP. Deferred post-CP15. |
| Session export manifest overwrite bug | Known | `session_f_run.py` opens shared `export_manifest.jsonl` in "w" mode per camera. Last camera overwrites earlier cameras' records. Needs "a" (append) mode or per-camera manifest files. Not blocking — all clips export correctly, only manifest records for earlier cameras are lost. |
...
- **Last updated:** 2026-03-24 (CP15: session pipeline E2E fixes — 7 fixes applied, session Phase 2 verified with .session_completed sentinel, uploader writes global_person_id to DB, cross-camera dedup fixed to per-camera scoping)

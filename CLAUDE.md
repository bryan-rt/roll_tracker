...
| CP16b: Calibration pipeline skeleton + functional lens calibration | Completed | `src/calibration_pipeline/` created with `lens_calibration.py` (functional), `mat_walk.py`, `drift_detection.py`, `inter_camera_sync.py` (stubs → CP18). Lens calibration: auto-detects mat edge points via 1D gradient analysis along perpendicular profiles (50–100+ points/edge with sub-pixel precision), user can add/delete points manually. `cv2.calibrateCamera` with `CALIB_FIX_TANGENT_DIST | CALIB_FIX_PRINCIPAL_POINT | CALIB_FIX_ASPECT_RATIO` → 3 free params (f, k1, k2). Quality filters reject weak gradients and outliers. Writes `camera_matrix`, `dist_coefficients`, `lens_calibration` metadata (incl. auto/manual point counts, per-edge stats) to homography.json. `homography_calibrate.py` updated: (1) `_write_homography_json()` read-then-merges to preserve K+dist, (2) auto-undistorts frame at load when K+dist present (both UI modes). `calibration_pipeline` in `pyproject.toml` packages. Two-step calibration chain: lens_calibration → K+dist, then homography_calibrate on undistorted frame → H. |

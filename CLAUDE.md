...
| Coast architecture: variable-dt Kalman | **Implemented (Piece 11) — T1+T2 PASS, blocked on MUXER-PTS-1 for full corpus** | Subclass (not fork) of boxmot's `KalmanFilterXYWH` and `BotSort` at `src/bjj_pipeline/tracking/`. Uses dt ratio (`dt_s / nominal_dt_s`) to keep velocity in px/nominal-frame. Both KF sites replaced (`self.kalman_filter` + `STrack.shared_kalman`). Track lifetime via wall-time `max_lost_seconds` (default 2.0s = today's behavior) — `frame_rate` eliminated. Toggle: `variable_dt: true/false`. T1 PASS (constant-cadence = stock bit-for-bit). T2 PASS (A→F unimodal, A→D bimodal — Stage E failure is pre-existing CP22, confirmed identical with variable_dt=false). Piece 8 dissolved. T3 deferred to CP-R8 annotation. Process-noise dt-scaling is a recorded follow-up (second-order under ratio formulation). |

...
| Gym setup calibration tool | Planned | `tools/detect_buzzer.py` exists as a standalone audio survey tool. The production gym calibration system is being built as `src/calibration_pipeline/` (CP16b+). CP16b builds the functional lens calibration module and pipeline skeleton. CP18 builds mat walk, drift detection, and inter-camera sync. Buzzer detection runs inside the per-session pipeline (Phase 1, Stage A.5 — future). AprilTag visibility heatmap is a CP18 output surfaced in `app_web`. |
...
| Cross-camera ILP enhancement | Planned (CP17) | Current merge is post-hoc (links resolved person_ids after per-camera ILP). Future: second ILP pass feeds cross-camera tag co-observations back as must_link constraints into per-camera D3 before solving — enables resolving within-camera identity swaps using cross-camera evidence. See CP17 entry below. |
...
| CP16a: F0 projection utility + call site migration | Planned | Audit all homography call sites in `src/bjj_pipeline`. Build `project_to_world(pixel_xy, camera_id)` utility in F0 contract layer. Utility loads K + distortion coefficients per camera from local config. Applies `cv2.undistortPoints()` then homography (Option B). Migrate every existing call site — no stage bypasses the utility. Define local config format for K + distortion coefficients. With identity placeholder values (no real K + distortion coefficients until CP16b), CP16a validation confirms the utility code path is exercised correctly and pipeline output is unchanged — not a quality improvement test. Quality shift for J_EDEw becomes testable in CP16b. Debug artifact written to `_debug/` folder with before/after world coordinates for diffing. |
| CP16b: Calibration pipeline skeleton | Planned | Create `src/calibration_pipeline/` as top-level module alongside `src/bjj_pipeline`. Module stubs: `mat_walk`, `drift_detection`, `inter_camera_sync` — each with clear docstrings defining inputs, outputs, and intended algorithm. Storage contracts: Supabase schema additions for K + distortion coefficients (extends `homography_configs` or new `camera_calibrations` table — decided in CP16b), local config layout adds distortion fields to existing `configs/cameras/{cam_id}/homography.json`. `lens_calibration` module is FUNCTIONAL in CP16b: uses existing homography overlay tool output (corner points + mat edge correspondences) to estimate K + distortion coefficients via OpenCV calibration solver, displays corrected frame with overlay aligned for user verification, writes real K + distortion values to `homography.json`. Wire skeleton so CP16a utility reads these values immediately when present. No mat walk grid pattern detection — that is CP18. |
| CP17: Second ILP layer (tag signal, coordinate-ready) | Planned | Second ILP pass feeds cross-camera AprilTag co-observations back as must_link constraints into per-camera D3 before solving. Tag co-observation channel fully implemented. World coordinate spatial-temporal evidence channel stubbed but architecturally present — accepts None until CP18 activates it. Testable against existing 36 clips: sparse tag pings for one person across two cameras should show improved tracklet continuity. |
| CP18: Calibration pipeline fleshed out | Planned | Mat walk logic fully implemented: grid pattern detection, labeled correspondence extraction (single known tagged person, known world coordinates), inter-camera affine alignment via least-squares solve on labeled correspondences across cameras. Refined homography written to Supabase and local config. Drift detection: empty-mat baseline snapshot taken immediately after successful calibration, daily frame comparison via edge detection, drift score to Supabase. Severe drift alert: notify gym owner if drift exceeds hard threshold AND recalibration quality is poor. Scheduled recalibration via `nest_recorder` secondary pipeline window. Baseline snapshot tied to successful calibration run. Activates world coordinate evidence channel in CP17 architecture. |
...
- **Next:** CP16a — F0 projection utility + call site migration.
...
- **Last updated:** 2026-03-25 (CP15 closed, post-CP15 manifest overwrite fix, checkpoint map CP16–CP18 added, CP14 migrations applied to remote Supabase)

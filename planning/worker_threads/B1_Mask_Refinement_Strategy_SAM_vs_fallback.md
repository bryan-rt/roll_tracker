...
## Module-specific context (B1 — mask strategy)
SAM is an offline refinement tool. For POC, we may:
- run SAM only for tracklet frames likely to contain tags or for uncertain regions
- keep a fallback mask/box path

### Must include
- When to run SAM vs use lighter masks (YOLO-seg) vs boxes
- Prompting strategy from tracklets
- Caching + deterministic mask IDs
- Quality scoring of masks (connected components, area sanity)

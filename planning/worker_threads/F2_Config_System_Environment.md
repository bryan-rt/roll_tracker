...
## Module-specific context (F2 — config/environment)
We need configs that encode:
- camera geometry (homography)
- thresholds (match start/end, gating)
- compute toggles (use_sam, sample_frames_for_tags, etc.)

### Must include
- `configs/default.yaml` with sensible POC values
...
## 🔒 Clarification (F1 Lock)

Config merge semantics are **authoritative and final**:

1. `configs/default.yaml`
2. `configs/cameras/<camera_id>.yaml`
3. CLI `--config` overlay

Deep merge, later values override earlier ones.

This worker must not redefine merge behavior.

## Completion report

Status: ✅ COMPLETE  
Verified via: real pipeline run + audit inspection (config_resolved events)

### Final locked behaviors (authoritative)

- **camera_id is explicit and canonicalized**
  - `config["camera_id"] == <runtime camera_id>`
  - `config["camera"]["camera_id"] == <runtime camera_id>`
- **Config merge order (deep merge, later wins)**
  1) `configs/default.yaml`  
  2) `configs/cameras/<camera_id>.yaml` (if present)  
  3) CLI `--config` overlay (YAML or JSON)
- **Homography config (camera-scoped)**
...
- `src/bjj_pipeline/config/models.py` — typed config models (unknown keys forbidden)
- `src/bjj_pipeline/config/loader.py` — deep merge, canonicalization, hashing
- `src/bjj_pipeline/config/paths.py` — cache path helpers
- Orchestration integration:
  - `src/bjj_pipeline/stages/orchestration/cli.py`
  - `src/bjj_pipeline/stages/orchestration/pipeline.py`
- Audit artifact:
  - `outputs/<clip_id>/orchestration_audit.jsonl` (includes `config_resolved`)

### Downstream assumptions (safe)

- No stage should implement its own config merge logic.
- All stages receive a finalized, canonicalized `config` from orchestration.
- If homography is missing, that is intentional and must be handled upstream (preflight / calibrator), not inside stages.

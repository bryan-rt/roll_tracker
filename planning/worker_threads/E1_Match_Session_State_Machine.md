# E1 — Match Session State Machine
...
## Module-specific context (E1 — match session state machine)
This turns stitched tracks into sessions.

### Must include
- Hysteresis:
  - Start when distance < D_start for T_start
  - End when distance > D_end for T_end
  - Gap tolerance to prevent short breaks splitting sessions
- Partner rotation logic (A-B then A-C)
- Confidence/evidence: which frames supported start/end

### Acceptance tests
- Prevent stutter clips
- Correctly splits A-B then A-C

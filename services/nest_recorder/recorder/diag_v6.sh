...
build_ffmpeg_opts() {
  ...
  if [ "$REENCODE" = "1" ] || [ "$REENCODE" = "2" ]; then
    ...
    if [ "$FPS_PASSTHROUGH" = "1" ]; then
      ...
      # PASSTHROUGH ONLY — under CFR, -enc_time_base 1/90000 breaks the segment muxer's
      # cut-point calculation, producing a single unsegmented file instead of SEG_SECONDS
      # segments. CFR resamples to a uniform grid and does not need PTS precision preserved.
...
extract_timing_sidecars() {
  ...
  for (( si=0; si<${#seg_paths[@]}; si++ )); do
    ...
    # mismatch is structurally false under mp4-derived generation (schema 5).
    # Retained for the CFR awk which still uses it in variable passing.

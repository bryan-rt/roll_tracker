...
## Module-specific context (X2 — opt-in redaction)
Opt-in is driven by AprilTag->athlete mapping.

### Must include
- Policy rules:
  - both opted-in => save normally
  - one opted-out => redact that person in all frames
  - both opted-out => discard or metadata-only (POC choice)
- Implementation strategy: mask-based blur/blackout (can start coarse, upgrade)
- Audit log entries describing decisions
